...
### ✅ Frontend (`feat/multi-tenancy-frontend`) — done

**Update 5 (current):** Client-side work for Phase 1 is complete: `RegisterView.vue`, login-time tenant picker, NavBar "switch workspace" dropdown, `useAuth` additions (`isOwner`, `canManageBudget`, `memberships`, `register`, `selectTenant`, `fetchMemberships`), `canManageBudget` replacing `isHead` in `CategoriesView`/`TransactionsView`, `CategoryForm.vue`'s personal-tenant branch (hides department selection) plus a new inline department-creation affordance. `client && npm run build`/`lint` clean; `server && npm test` 177/177, build/lint clean. Manually verified end-to-end in the browser: personal registration → clean dashboard with no department concepts; company registration → head creates a department inline → creates a category in it; join-by-code registration → employee, unassigned department; join-by-code using an email that already holds a personal membership → login-time tenant picker renders and works; NavBar workspace switcher → switches active tenant and the UI updates correctly. Docs written: `docs/stories/16-multi-tenancy-frontend.md`, `docs/decisions/16-multi-tenancy-frontend.md`, `docs/FILE_STRUCTURE.md` updated.

Two bugs found while doing this manual verification, both documented in decision 16:
- **Fixed, on `feat/multi-tenancy-foundation` (PR #7) directly, not this branch:** `POST /api/departments` never granted the creating head `department_access` to the department they just created — a fresh company head could create a department through the new UI and then immediately get 403'd trying to use it. This branch was rebased onto the fixed foundation commit afterward.
- **Known gap, deliberately left open:** a personal-tenant `owner` can set `approval_threshold` on their own categories in the UI, but `/api/approvals` is `department_head`-only — a personal-tenant transaction that trips its threshold has no route any personal-tenant user can call to approve it. Not patched in either direction (see decision 16 for why) — flagging here for whoever picks up personal-tenant approval behavior.

**Deferred, not part of this phase** (per the "routing infrastructure" open question in the original handoff): no separate enterprise-vs-personal route tree or NavBar variant was built. Phase 2's personal-specific views (Accounts/Paycheck/Goals/Debts) don't exist yet, so there's nothing for a route split to switch to yet — the existing Dashboard/Transactions/Categories views work correctly for `owner` via the `canManageBudget`/`isPersonal` generalizations alone, verified manually. Revisit when Phase 2 views are actually being built.

**Still outstanding, explicitly requested by the user, not started:** a standalone doc explaining the general concept of multi-tenancy and this implementation's specific approach — call out which parts are standard/textbook (tenant table, membership table, per-request scoping) vs. stack-specific (SQLite `DROP COLUMN` migration, JWT-in-cookie, Express middleware, the pre-tenant-token bridge for multi-membership login).

**Also deferred, explicitly noted by the user for later** (not part of this phase): dashboard filtering by department and by specific budget — do this *after* the personal-use multi-tenancy work is fully done.
